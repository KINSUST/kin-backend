...
const allPost = asyncHandler(async (req, res) => {
  ...
  if (!posts.length) throw createError(400, "Couldn't find any data.");

  //  count
  const total = await Post.findAll()

  const totalLength = total.length || 0
  ...
  // pagination object
  const pagination = {
    totalDocuments: totalLength,
    totalPages: Math.ceil(totalLength / limit),
    currentPage: page,
    previousPage: page > 1 ? page - 1 : null,
    nextPage: page < Math.ceil(totalLength / limit) ? page + 1 : null,
  };
    ...
    include: [
      {
        model: Comment,
        as: "comments",
        attributes: ["id", "comment", "post_id", "email","name"],
      },
    ],
  });
...
const commentOnPostById = asyncHandler(async (req, res) => {
  ...
  if (!post) throw createError(400, "Couldn't find any post data.");
    ...
    include: [
      {
        model: Comment,
        as: "comments",
        attributes: ["id", "comment", "post_id","name"],
      },
    ],
  });
...
/**
 * @method         DELETE
 * @description    Delete comment on single post data
 * @route          /api/v1/posts/delete-comment/:id
 */
...
const deleteCommentOnPostById = asyncHandler(async (req, res) => {
  ...
  // if no data found
  if (!comment) throw createError(400, "Couldn't find any comment data.");
